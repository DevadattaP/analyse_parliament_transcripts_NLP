...
Recent government initiatives such as the Sansad Bhashini project highlight the growing importance of applying Natural Language Processing (NLP) techniques to parliamentary data for improving accessibility, searchability, and real-time analysis. Inspired by this direction, our project focuses on a fundamental sub-task in this pipeline: identifying the relevant ministry for a given paragraph of parliamentary text, even when the ministry name is not explicitly mentioned.
...
\begin{itemize}
    \item Use LLM-generated labels (ChatGPT) as a weak reference signal (for few test documents)
...
\begin{enumerate}
    ...
    \item Documents are checked against a parliamentary vocabulary list to ensure they are relevant to parliamentary proceedings.
...
\begin{itemize}
    ...
    \item Domain-specific stopwords (common parliamentary terms)
    \item Additional stopwords identified during topic modeling (using David Mimno’s online tool)
\end{itemize}
...
\subsection{Ministry Embedding 2 (Average of Paragraph Embeddings)}
...
Instead of training a local topic model, we used the online tool by \textit{David Mimno} to perform topic modeling on the corpus. This tool provides set number of topics, top representative words for each topic, document–word–topic state over complete vocabulary, stopwords, and topic coherence scores, etc.
...
\begin{enumerate}
    \item \textbf{High Semantic Overlap in Knowledge Base}\\
        ...
        \end{itemize}
        As a result, many ministries share common vocabulary and overlapping semantic content. For example:
        \begin{itemize}
    ...
    \item \textbf{Uncertainty in Embedding Dimensionality}\\
        ...
        \end{itemize}
        Even when visualized using dimensionality reduction techniques such as PCA and t-SNE, some separation between ministry clusters is visible, but not strong enough for reliable classification.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Each paragraph was labeled using an LLM (ChatGPT), serving as a reference signal
    ...
    \item Predictions were compared against reference labels using:
        \begin{itemize}
            ...
            \item Top-k error counts (k = 2, 3, 4, 5)
            \item Cluster-level matches
            \item Topic-family matches
        \end{itemize}
\end{itemize}
...
Embedding-based approaches significantly outperform vocabulary and probabilistic methods in exact classification. In particular, ministry\_embedding\_v2 (paragraph-level averaging) achieves the best balance between representation and generalization.
...
% add linked for the data and tools used, and the references
\end{document}
